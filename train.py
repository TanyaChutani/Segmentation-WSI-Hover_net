...
class Dice_Loss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        return tf.nn.softmax_cross_entropy_with_logits(y_true_f, y_pred) + (
            1 - tf.math.reduce_mean((2.0 * intersect / (denom)))
        )
...
class Dice_Metric(tf.keras.metrics.Metric):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        y_pred_f = tf.nn.softmax(y_pred)
        y_pred_c = tf.cast(
            tf.equal(tf.math.argmax(y_pred_f, axis=-1), self.class_id), dtype=tf.float32
        )

        intersect_c = tf.math.reduce_sum(y_true_c * y_pred_c)
        denom_c = tf.math.reduce_sum(y_true_c + y_pred_c)

        self.dice_metric_c.assign(tf.math.reduce_mean((2.0 * intersect_c / (denom_c))))

    def result(self):
        return self.dice_metric_c

    def reset_states(self):
        self.dice_metric_c.assign(0.0)
...
def main():
    ...
    batch_size = 4
    num_classes = 3
    train_images_folder = os.listdir("/home/ubuntu/aira/consep/consep/train/images/")
    valid_images_folder = os.listdir("/home/ubuntu/aira/consep/consep/valid/images/")

    train_imgs_folder = sorted(train_images_folder)
    valid_imgs_folder = sorted(valid_images_folder)

    train_img_folder = list(
        map(
            lambda orig_string: "/home/ubuntu/aira/consep/consep/train/images/"
            + orig_string,
            train_imgs_folder,
        )
    )
    train_mask_folder = list(
        map(
            lambda orig_string: "/home/ubuntu/aira/consep/consep/train/masks/"
            + orig_string,
            train_imgs_folder,
        )
    )
    valid_img_folder = list(
        map(
            lambda orig_string: "/home/ubuntu/aira/consep/consep/valid/images/"
            + orig_string,
            valid_imgs_folder,
        )
    )
    valid_mask_folder = list(
        map(
            lambda orig_string: "/home/ubuntu/aira/consep/consep/valid/masks/"
            + orig_string,
            valid_imgs_folder,
        )
    )
