...
import albumentations as A
from matplotlib import cm
from load_dataset import PanNukeDataModule
...
pannuke = PanNukeDataModule(
    data_dir="/home/ubuntu/aira/consep/consep/train/",
    download=False,
...
n = 2
fig, ax = plt.subplots(nrows=n, ncols=2, figsize = (8, 8))
...
for i in range(n):
    ...
    m = np.argmax(masks,axis=1)[i, ...]
    m = np.ma.masked_where(m == 0, m)
    ax[i, 1].imshow(m)

for a in ax.ravel(): a.axis()
for c,v in enumerate(["H&E Image", "Nucleus Types"]):
    ax[0, c].set_title(v)
plt.tight_layout()
plt.show()
